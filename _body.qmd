```{r}
nextRender <- (lubridate::now() + lubridate::hours(4)) |> 
  lubridate::with_tz(tzone = 'Europe/Vienna')
```

<small>Hinweis: Die Seite wird circa alle 4 Stunden mit neuen Daten befüllt. Nächstes Update um circa `r nextRender`.</small>
...
Als Beispiel können wir mit den iNaturalist Daten und nur Beobachtungen auf zumindest Spezies Niveau den Shannon-Index *H`* berechnen: **`r round(shannonDiversity$shannonDiversity[1], 2)`**. Die Anzahl der Arten beträgt **`r shannonDiversity$speciesRichness[1]`**. 

Weiters können wir mit diesen zwei Kennzahlen den Evenness-Index berechnen, welcher einen Wert von **`r round(shannonDiversity$shannonDiversity[1] / log(shannonDiversity$speciesRichness[1]), 2)`** ergibt.
...
getIdentifierAsVector <- function(identifications, observerId){
  if(!is.null(identifications) & identifications != 'list()'){
