...
# Birds RedList
birdsList <- read_csv2("data/birds.csv", show_col_types = FALSE) |>
    rename_with(~"scientific_name", 1) |>
    select(scientific_name, `Annex I`) |>
    rename(AnnexI = `Annex I`) |>
    drop_na(AnnexI)
# German Common Names
taxasDE <- read_csv('data/VernacularNames-german.csv', show_col_types = FALSE) |> 
    group_by(id) |> 
    summarise(
...
taxas <- read_csv('data/taxa.csv', show_col_types = FALSE) |> 
   ...
   tryCatch(
     exp = {
      ...
      if(checkIfFileExists){
        ...
        obsResults <- read_csv(glue("data/obs_{projectId}.csv"), show_col_types = FALSE)
        return(obsResults)
      }
      apiEndpoint <- "https://api.inaturalist.org/v2/observations"
      fields <- "(id:!t,uri:!t,quality_grade:!t,num_identification_disagreements:!t,user:(name:!t,id:!t,login:!t),taxon:(id:!t))"
      perPage <- 200
