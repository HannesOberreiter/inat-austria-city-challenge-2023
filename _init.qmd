...
loadObs <- function(projectId){
   tryCatch(
     exp = {
      ...
      obsList <- list()
      fields <- "(uri:!t,quality_grade:!t,num_identification_disagreements:!t,user:(name:!t,id:!t,login:!t),taxon:(id:!t))"
      for (x in 1:totalPages) {
        ...
        # maybe separate into phylums to not run into limit issue as 100 would be again problematic
        if(x < 50){
          nextPage <- x
          direction <- "desc"
        } else if(x > 100){
      ...
      obsResults <- bind_rows(obsList) %>%
        ...
        mutate(
          user.name = ifelse(is.na(user.name), user.login, user.name)
        ) %>%
